...
c     longitude slice of the ionization rates
      ...
      open(unit=111, file="long_slice_180"//yes_no//".txt")
      open(unit=116, file="long_slice_270"//yes_no//".txt")
      open(unit=117, file="alt_slice_k-10"//yes_no//".txt")
      do I=1,91           !long
         do J=1,20        !lat
            do k=1,15     !height
...
c            if (K .eq. 15)then
c     create xyz geo coordinates                                                                  
c            the=(I-1)*2.*(3.14159/180)
c            phi=(mod((((j-1)*18.)+180.0),360.0)-180)*(3.14159/180)
c            R=6731+(zkmheight(K,I,J)/1000)
c            xgeo=(6731+(zkmheight(K,I,J)/1000))*sin(the)*cos(phi)
c            ygeo=(6731+(zkmheight(K,I,J)/1000))*sin(the)*sin(phi)
c            zgeo=(6731+(zkmheight(K,I,J)/1000))*cos(the)
c     print out a polar profile                                                                     
            
c            call ctim_cotr('geo','gse',xgeo,ygeo,zgeo,xgse,ygse,zgse)
